            ...
            if (pnmv->lParam != -1) {
				...
				if ((GetKeyState(VK_SHIFT) & 0xff00) == 0) {
					...
					for (i = 0; i < options.image_count; i++) {
						...
						mess_image_nums[i] = -1;
						ListView_Update(hwndImages, imagenum);
					}
					MessRemoveImage(-1);
				}
			}
...
static void FillSoftwareList(int nGame)
{
	...
	ListView_SetItemCount(hwndImages, mess_images_count);
...
static void OnMessIdle()
{
	...
	for (i = 0; (i < 10) && (nIdleImageNum < mess_images_count); i++) {
		...
		if (pImageData->type == IO_ALIAS) {
			...
			ListView_RedrawItems(hwndImages,nIdleImageNum,nIdleImageNum);
		}
		nIdleImageNum++;
	}
