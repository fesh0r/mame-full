...
Each driver which uses this must use svi318dsk_set_geometry, so that
...
#define svi318dsk_MAX_DRIVES 4
#define VERBOSE 1
static svi318dsk     svi318dsk_drives[svi318dsk_MAX_DRIVES];

static void svi318dsk_seek_callback(int,int);
static int svi318dsk_get_sectors_per_track(int,int);
static void svi318dsk_get_id_callback(int, chrn_id *, int, int);
static void svi318dsk_read_sector_data_into_buffer(int drive, int side, int index1, char *ptr, int length);
static void svi318dsk_write_sector_data_from_buffer(int drive, int side, int index1, char *ptr, int length,int ddam);

floppy_interface svi318dsk_floppy_interface=
{
	svi318dsk_seek_callback,
	svi318dsk_get_sectors_per_track,             /* done */
	svi318dsk_get_id_callback,                   /* done */
	svi318dsk_read_sector_data_into_buffer,      /* done */
	svi318dsk_write_sector_data_from_buffer, /* done */\
	NULL,
	NULL
};
...
void svi318dsk_set_ddam(UINT8 id, UINT8 physical_track, UINT8 physical_side, UINT8 sector_id,UINT8 ddam)
{
	...
	logerror("svi318dsk_set_ddam: #%d T:$%02x H:%d S:$%02x = %d\n",id, physical_track, physical_side, sector_id,ddam);
...
static int svi318dsk_get_ddam(UINT8 id, UINT8 physical_track, UINT8 physical_side, UINT8 sector_id)
{
	unsigned long ddam_bit_offset, ddam_bit_index, ddam_byte_offset;
	svi318dsk *pDisk;
...
void svi318dsk_set_geometry(UINT8 drive, UINT16 tracks, UINT8 heads, UINT8 sec_per_track, UINT16 sector_length, UINT8 first_sector_id, UINT16 offset_track_zero)
{
	svi318dsk *pDisk;
	unsigned long N;
	...
	if (drive >= svi318dsk_MAX_DRIVES)
	{
		logerror("svi318dsk drive #%d not supported!\n", drive);
		return;
	}

	pDisk = &svi318dsk_drives[drive];


#if VERBOSE
	logerror("svi318dsk geometry for drive #%d is %d tracks, %d heads, %d sec/track, %d bytes per sector, first sector id: %d, file offset to track 0: %d\n",
...
static int svi318dsk_seek(svi318dsk * w, UINT8 t, UINT8 h, UINT8 s)
{
unsigned long offset;
	/* allow two additional tracks */
    if (t >= w->tracks + 2)
	{
		logerror("svi318dsk track %d >= %d\n", t, w->tracks + 2);
		return 0;
	}

    if (h >= w->heads)
    {
		logerror("svi318dsk head %d >= %d\n", h, w->heads);
		return 0;
	}

    if (s >= (w->first_sector_id + w->sec_per_track))
	{
		logerror("svi318dsk sector %d\n", w->sec_per_track+w->first_sector_id);
		return 0;
	}
...
//	offset += w->offset;
...
#if VERBOSE
    logerror("svi318dsk seek track:%d head:%d sector:%d-> offset #0x%08lX\n",
             t, h, s, offset);
#endif

	if (offset > w->image_size)
	{
		logerror("svi318dsk seek offset %ld >= %ld\n", offset, w->image_size);
		return 0;
	}
	...
	if (w->dam_cnt)
	{
		...
		for (i = 0; i < w->dam_cnt; i++)
		{
			...
			{
#if VERBOSE
                                logerror("svi318dsk reading formatted sector %d, track %d, head %d\n", w->sector, w->track, w->head);
#endif
...
#if 0
/* write a sector */
static void svi318dsk_write_sector(svi318dsk *w)
{
...
static void svi318dsk_write_track(svi318dsk *w)
{
...
int cnt;
    ...
    {
#if VERBOSE
                logerror("svi318dsk write_track write protected image\n");
#endif
	...
	f = w->buffer;
#if VERBOSE
        logerror("svi318dsk write_track %s_LOW\n", (w->density) ? "MFM" : "FM" );
#endif
	...
	do
	{
		...
		int seclen;
...
#if VERBOSE
                        logerror("svi318dsk write_track FE @%5d T:%02X H:%02X S:%02X L:%02X\n",
					(int)(f - w->buffer),
			...
			if (cnt > seclen)
			{
				...
				w->dam_cnt++;
#if VERBOSE
                                logerror("svi318dsk write_track %02X @%5d data: %02X %02X %02X %02X ... %02X %02X %02X %02X\n",
						f[-1],
						(int)(f - w->buffer),
						f[0], f[1], f[2], f[3],
...
void    svi318dsk_get_id_callback(int drive, chrn_id *id, int id_index, int side)
{
	svi318dsk *w = &svi318dsk_drives[drive];
	...
	/* get dam */
	if (svi318dsk_get_ddam(drive, w->track, side, id->R))
	{
...
int  svi318dsk_get_sectors_per_track(int drive, int side)
{
	svi318dsk *w = &svi318dsk_drives[drive];
...
void    svi318dsk_seek_callback(int drive, int physical_track)
{
	svi318dsk *w = &svi318dsk_drives[drive];
...
void svi318dsk_write_sector_data_from_buffer(int drive, int side, int index1, char *ptr, int length, int ddam)
{
	svi318dsk *w = &svi318dsk_drives[drive];
...
void svi318dsk_read_sector_data_into_buffer(int drive, int side, int index1, char *ptr, int length)
{
	svi318dsk *w = &svi318dsk_drives[drive];
