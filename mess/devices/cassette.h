...
#define CONFIG_DEVICE_CASSETTE(count, formats)									\
	...
	{																			\
		...
		cfg->dev = cassette_device_specify(&iodev, extbuf, sizeof(extbuf) /		\
			sizeof(extbuf[0]),	(count), (formats), NULL, (cassette_state) -1);	\
	}																			\

#define CONFIG_DEVICE_CASSETTEX(count, formats, create_opts, default_state)		\
	if (cfg->device_num-- == 0)													\
	{																			\
		static struct IODevice iodev;											\
		static char extbuf[33];													\
		cfg->dev = cassette_device_specify(&iodev, extbuf, sizeof(extbuf) /		\
			sizeof(extbuf[0]),	(count), (formats), (create_opts),				\
			(default_state));													\
	}																			\
