...
void    EventList_AddItemOffset(int ID, int Data, int Time)
{
        ...
        if (!CyclesPerFrame)
                CyclesPerFrame = (int)(Machine->drv->cpu[0].cpu_clock / Machine->drv->screen[0].refresh_rate);	//totalcycles();	//_(int)(Machine->drv->cpu[0].cpu_clock / Machine->drv->frames_per_second);
