...
/*
  ...
  missing grid, sound generation, position readback, retriggered sprites, paddle reading
  score display

  less encapsuled
 */

static UINT8 sprite_collision[0x7f];
static UINT8 background_collision[0x7f];
...
int vc4000_vh_start(void)
{
	...
	if ((i&0x11)==0x11) sprite_collision[i]|=0x80;
	if ((i&0x12)==0x12) sprite_collision[i]|=0x40;
	if ((i&0x14)==0x14) sprite_collision[i]|=0x20;
	if ((i&0x18)==0x18) sprite_collision[i]|=0x10;
    }
...
WRITE_HANDLER(vc4000_video_w)
{
	...
	vc4000_video.sprites[nr].reloaded=TRUE;
...
//	if (vc4000_video.line!=This->data->y1) break;
	...
	ui_text(Machine->scrbitmap, message, 150, _y);_y+=10;
#endif
...
//		This->finished=TRUE;
	...
	break;
    case 12:
//	if (vc4000_video.line!=This->data->y2) break;
//	if (This->data->x2==0xff) {
//	    This->state=0;
//	    break;
//	}
	if (This->y!=0) {
	...
	ui_text(Machine->scrbitmap, message, 150, _y);
	_y+=10;
#endif
	This->state++;
	...
	if (vc4000_video.line<bitmap->height
	    ...
	    vc4000_collision_plot(collision+This->data->x1, This->data->bitmap[This->state-13],
				  This->mask,This->size);
...
INLINE void vc4000_draw_grid(UINT8 *collision)
{
    ...
    if (vc4000_video.line>=200) return;
    if (!vc4000_video.reg.d.background&8) return;

    i=(vc4000_video.line/20)*2;
    if (vc4000_video.line%20>=2) i++;
    ...
    for (x=0, j=0, m=0x80; j<16; j++, x+=8, m>>=1) {
...
int vc4000_video_line(void)
{
	...
	for (int i=0; i<4; i++) {
	    ...
	    ui_text(Machine->scrbitmap, message, 220, i*8);
	}
#endif
    }

//    if (vc4000_video.line<Machine->scrbitmap->height)
//	plot_box(Machine->scrbitmap, 0, vc4000_video.line, Machine->scrbitmap->width, 1, 
//		 Machine->pens[(vc4000_video.reg.d.background)&7]);
    ...
    cpu_set_irq_line(0, S2650_INT_IRQ, vc4000_video.line==250?1:0);
...
void vc4000_draw_digit(struct osd_bitmap *bitmap, int x, int y, int d)
{
    static int digit_to_segment[0x10]={ 
	0x3f, 6, 0x5b, 0x4f, 0x66, 0x6d, 0x7d, 7, 0x7f, 0x6f
    };
    int i,j;
    for (i=0; i<ARRAY_LENGTH(led); i++) {
...
void vc4000_vh_screenrefresh (struct osd_bitmap *bitmap, int full_refresh)
{
    ...
    vc4000_draw_digit(bitmap, x, y, vc4000_video.reg.d.bcd[0]>>4);
    vc4000_draw_digit(bitmap, x+16, y, vc4000_video.reg.d.bcd[0]&0xf);
    if (vc4000_video.reg.d.score_control&2) x+=20;
    vc4000_draw_digit(bitmap, x+32, y, vc4000_video.reg.d.bcd[1]>>4);
    vc4000_draw_digit(bitmap, x+48, y, vc4000_video.reg.d.bcd[1]&0xf);
