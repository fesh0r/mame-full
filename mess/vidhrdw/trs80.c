...
VIDEO_UPDATE( trs80 )
{
	...
	/* have we changed the tilemap? */
	new_tilemap = (trs80_port_ff & 0x08) ? dblwidth_tilemap : normwidth_tilemap;
	if (current_tilemap != new_tilemap)
	{
		current_tilemap = new_tilemap;
		tilemap_mark_all_tiles_dirty(current_tilemap);
	}
...
WRITE8_HANDLER( trs80_videoram_w )
{
	if (videoram[offset] != data)
	{
		...
		if (current_tilemap)
			tilemap_mark_tile_dirty(current_tilemap, offset);
	}
}
