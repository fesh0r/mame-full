...
static void snes_update_line_mode7( UINT8 screen, UINT16 curline )
{
	...
	for( sx = 0; sx < 256; sx++, xpos += xdir )
	{
		...
		if( scanlines[screen].zbuf[xpos] < table_bgd_pty[0][priority] && colour > 0 )
		{
			...
			scanlines[screen].zbuf[xpos] = table_bgd_pty[0][priority];
		}
	}
}
...
void snes_refresh_scanline( UINT16 curline )
{
	...
	if( curline == 0 )
	{
		...
		sprintf( t, "%s1 %s%s%s%s%s%c%s%s%d  %4X %4X",
		...
		sprintf( t, "%s2 %s%s%s%s%s%c%s%s%d  %4X %4X",
		...
		ui_text( Machine->scrbitmap, t, 300, y++ * 9 );
		sprintf( t, "%s3 %s%s%s%s%s%c%s%s%d%s %4X %4X",
		...
		ui_text( Machine->scrbitmap, t, 300, y++ * 9 );
		sprintf( t, "%s4 %s%s%s%s%s%c%s%s%d  %4X %4X",
