...
INPUT_PORTS_START( primo )
	...
	PORT_START_TAG( "IN3" )
		...
		PORT_BIT( 0x0008, IP_ACTIVE_HIGH, IPT_KEYBOARD ) PORT_NAME("£")		PORT_CODE(KEYCODE_END)		PORT_CHAR(UCHAR_MAMEKEY(F3))
		...
		PORT_BIT( 0x0400, IP_ACTIVE_HIGH, IPT_KEYBOARD ) PORT_NAME("‚")		PORT_CODE(KEYCODE_COLON)	PORT_CHAR(UCHAR_MAMEKEY(F4))
		PORT_BIT( 0x0800, IP_ACTIVE_HIGH, IPT_KEYBOARD ) PORT_NAME("¢")		PORT_CODE(KEYCODE_CLOSEBRACE)	PORT_CHAR(UCHAR_MAMEKEY(F5))
		PORT_BIT( 0x1000, IP_ACTIVE_HIGH, IPT_KEYBOARD ) PORT_NAME(" ")		PORT_CODE(KEYCODE_QUOTE)	PORT_CHAR(UCHAR_MAMEKEY(F6))
		PORT_BIT( 0x2000, IP_ACTIVE_HIGH, IPT_KEYBOARD ) PORT_NAME("Right")	PORT_CODE(KEYCODE_RIGHT)	PORT_CHAR(UCHAR_MAMEKEY(RIGHT))
		PORT_BIT( 0x4000, IP_ACTIVE_HIGH, IPT_KEYBOARD ) PORT_NAME("”")		PORT_CODE(KEYCODE_OPENBRACE)	PORT_CHAR(UCHAR_MAMEKEY(F7))
...
static void primo_snapshot_getinfo(const device_class *devclass, UINT32 state, union devinfo *info)
{
	...
	switch(state)
	{
		...
		case DEVINFO_STR_FILE_EXTENSIONS:				info->s = "pss\0"; break;
...
static void primo_quickload_getinfo(const device_class *devclass, UINT32 state, union devinfo *info)
{
	...
	{
		/* --- the following bits of info are returned as NULL-terminated strings --- */
		case DEVINFO_STR_FILE_EXTENSIONS:				info->s = "pp\0"; break;
