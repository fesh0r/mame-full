...
#if 0
	/* chargen */
	ROM_LOAD ("901460.03", 0x8000, 0x1000,CRC( 0x83e032a6))
	/* basic */
	ROM_LOAD ("901486.01", 0xc000, 0x2000,CRC( 0xdb4c43c1))
	/* kernel ntsc-m of vic1001? */
	ROM_LOAD ("901486.02", 0xe000, 0x2000,CRC( 0x336900d7))
	/* kernel ntsc */
	ROM_LOAD ("901486.06", 0xe000, 0x2000,CRC( 0xe5e7c174))
	/* kernel pal */
	ROM_LOAD ("901486.07", 0xe000, 0x2000,CRC( 0x4be07cb4))

	/* patched pal system for swedish/finish keyboard and chars */
	/* but in rom? (maybe patched means in this case nec version) */
	ROM_LOAD ("nec22101.207", 0x8000, 0x1000,CRC( 0xd808551d))
	ROM_LOAD ("nec22081.206", 0xe000, 0x2000,CRC( 0xb2a60662))

	/* ieee488 cartridge */
	ROM_LOAD ("325329-04.bin", 0xb000, 0x800,CRC( 0xd37b6335))
#endif
...
ROM_START (vic1001)
	ROM_REGION (0x10000, REGION_CPU1,0)
	ROM_LOAD ("901460.02", 0x8000, 0x1000,CRC( 0xfcfd8a4b))
	ROM_LOAD ("901486.01", 0xc000, 0x2000,CRC( 0xdb4c43c1))
	ROM_LOAD ("901486.02", 0xe000, 0x2000,CRC( 0x336900d7))
ROM_END
...
ROM_START (vic20i)
	ROM_REGION (0x10000, REGION_CPU1,0)
	ROM_LOAD ("901460.03", 0x8000, 0x1000,CRC( 0x83e032a6))
	ROM_LOAD ("325329.04", 0xb000, 0x800,CRC( 0xd37b6335))
	ROM_LOAD ("901486.01", 0xc000, 0x2000,CRC( 0xdb4c43c1))
	ROM_LOAD ("901486.06", 0xe000, 0x2000,CRC( 0xe5e7c174))
