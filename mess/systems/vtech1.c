...
INPUT_PORTS_START(vtech1)
    ...
    PORT_BIT(0x20, 0x00, IPT_KEYBOARD | IPF_RESETCPU) PORT_NAME("Reset") PORT_CODE(KEYCODE_F3)
    PORT_BIT( 0x1f, 0x1f, IPT_UNUSED )

    PORT_START /* IN1 KEY ROW 0 */
    PORT_BIT( 0xc0, IP_ACTIVE_LOW, IPT_UNUSED )
    PORT_KEY1(0x20, IP_ACTIVE_LOW, "R       RETURN  LEFT$",   KEYCODE_R,          CODE_NONE, 'R' )
    PORT_KEY1(0x10, IP_ACTIVE_LOW, "Q       FOR     CHR$",    KEYCODE_Q,          CODE_NONE, 'Q' )
    PORT_KEY1(0x08, IP_ACTIVE_LOW, "E       NEXT    LEN(",    KEYCODE_E,          CODE_NONE, 'E' )
	PORT_BIT( 0x04, IP_ACTIVE_LOW, IPT_UNUSED )
    PORT_KEY1(0x02, IP_ACTIVE_LOW, "W       TO      VAL(",    KEYCODE_W,          CODE_NONE, 'W' )
    PORT_KEY1(0x01, IP_ACTIVE_LOW, "T       THEN    MID$",    KEYCODE_T,          CODE_NONE, 'T' )

    PORT_START /* IN2 KEY ROW 1 */
    PORT_BIT( 0xc0, IP_ACTIVE_LOW, IPT_UNUSED )
    PORT_KEY1(0x20, IP_ACTIVE_LOW, "F       GOSUB   RND(",    KEYCODE_F,          CODE_NONE, 'F' )
    PORT_KEY1(0x10, IP_ACTIVE_LOW, "A       MODE(   ASC(",    KEYCODE_A,          CODE_NONE, 'A' )
    PORT_KEY1(0x08, IP_ACTIVE_LOW, "D               RESTORE", KEYCODE_D,          CODE_NONE, 'D' )
    PORT_KEY1(0x04, IP_ACTIVE_LOW, "L-CTRL",                  KEYCODE_LCONTROL,   CODE_NONE, UCHAR_SHIFT_2 )
    PORT_KEY1(0x04, IP_ACTIVE_LOW, "R-CTRL",                  KEYCODE_RCONTROL,   CODE_NONE, UCHAR_SHIFT_2 )
    PORT_KEY1(0x02, IP_ACTIVE_LOW, "S       STEP    SINS(",   KEYCODE_S,          CODE_NONE, 'S' )
    PORT_KEY1(0x01, IP_ACTIVE_LOW, "G       GOTO    STOP",    KEYCODE_G,          CODE_NONE, 'G' )

    PORT_START /* IN3 KEY ROW 2 */
    PORT_BIT( 0xc0, IP_ACTIVE_LOW, IPT_UNUSED )
    PORT_KEY1(0x20, IP_ACTIVE_LOW, "V       LPRINT  USR",     KEYCODE_V,          CODE_NONE, 'V' )
    PORT_KEY1(0x10, IP_ACTIVE_LOW, "Z       PEEK(   INP",     KEYCODE_Z,          CODE_NONE, 'Z' )
    PORT_KEY1(0x08, IP_ACTIVE_LOW, "C       CONT    COPY",    KEYCODE_C,          CODE_NONE, 'C' )
    PORT_KEY1(0x04, IP_ACTIVE_LOW, "L-SHIFT",                 KEYCODE_LSHIFT,     CODE_NONE, UCHAR_SHIFT_1 )
    PORT_KEY1(0x04, IP_ACTIVE_LOW, "R-SHIFT",                 KEYCODE_RSHIFT,     CODE_NONE, UCHAR_SHIFT_1 )
    PORT_KEY1(0x02, IP_ACTIVE_LOW, "X       POKE    OUT",     KEYCODE_X,          CODE_NONE, 'X' )
    PORT_KEY1(0x01, IP_ACTIVE_LOW, "B       LLIST   SOUND",   KEYCODE_B,          CODE_NONE, 'B' )

    PORT_START /* IN4 KEY ROW 3 */
    PORT_BIT( 0xc0, IP_ACTIVE_LOW, IPT_UNUSED )
    PORT_KEY2(0x20, IP_ACTIVE_LOW, "4  $    VERFY   ATN(",    KEYCODE_4,          CODE_NONE, '4', '$' )
    PORT_KEY2(0x10, IP_ACTIVE_LOW, "1  !    CSAVE   SIN(",    KEYCODE_1,          CODE_NONE, '1', '!' )
	PORT_KEY2(0x08, IP_ACTIVE_LOW, "3  #    CALL??  TAN(",    KEYCODE_3,          CODE_NONE, '3', '#' )
    PORT_BIT( 0x04, IP_ACTIVE_LOW, IPT_UNUSED )
    PORT_KEY2(0x02, IP_ACTIVE_LOW, "2  \"    CLOAD   COS(",   KEYCODE_2,          CODE_NONE, '2', '\"' )
    PORT_KEY2(0x01, IP_ACTIVE_LOW, "5  %    LIST    LOG(",    KEYCODE_5,          CODE_NONE, '5', '%' )

    PORT_START /* IN5 KEY ROW 4 */
    PORT_BIT( 0xc0, IP_ACTIVE_LOW, IPT_UNUSED )
    PORT_KEY1(0x20, IP_ACTIVE_LOW, "M       [Left]",          KEYCODE_M,          CODE_NONE, 'M' )
    PORT_KEY1(0x10, IP_ACTIVE_LOW, "SPACE   [Down]",          KEYCODE_SPACE,      CODE_NONE, ' ' )
    PORT_KEY1(0x08, IP_ACTIVE_LOW, ",       [Right]",         KEYCODE_COMMA,      CODE_NONE, ',' )
    PORT_BIT( 0x04, IP_ACTIVE_LOW, IPT_UNUSED )
    PORT_KEY1(0x02, IP_ACTIVE_LOW, ".       [Up]",            KEYCODE_STOP,       CODE_NONE, '.' )
    PORT_KEY1(0x01, IP_ACTIVE_LOW, "N       COLOR   USING",   KEYCODE_N,          CODE_NONE, 'N' )

    PORT_START /* IN6 KEY ROW 5 */
    PORT_BIT( 0xc0, IP_ACTIVE_LOW, IPT_UNUSED )
    PORT_KEY2(0x20, IP_ACTIVE_LOW, "7  '    END     ???",     KEYCODE_7,          CODE_NONE, '7', '\'' )
    PORT_KEY2(0x10, IP_ACTIVE_LOW, "0  @    ????    INT(",    KEYCODE_0,          CODE_NONE, '0', '@'  )
    PORT_KEY2(0x08, IP_ACTIVE_LOW, "8  (    NEW     SQR(",    KEYCODE_8,          CODE_NONE, '8', '(' )
    PORT_KEY2(0x04, IP_ACTIVE_LOW, "-  =    [Break]",         KEYCODE_MINUS,      CODE_NONE, '-', '=' )
    PORT_KEY2(0x02, IP_ACTIVE_LOW, "9  )    READ    ABS(",    KEYCODE_9,          CODE_NONE, '9', ')' )
    PORT_KEY2(0x01, IP_ACTIVE_LOW, "6  &    RUN     EXP(",    KEYCODE_6,          CODE_NONE, '6', '&' )

    PORT_START /* IN7 KEY ROW 6 */
    PORT_BIT( 0xc0, IP_ACTIVE_LOW, IPT_UNUSED )
    PORT_KEY1(0x20, IP_ACTIVE_LOW, "U       IF      INKEY$",  KEYCODE_U,          CODE_NONE, 'U' )
    PORT_KEY1(0x10, IP_ACTIVE_LOW, "P       PRINT   NOT",     KEYCODE_P,          CODE_NONE, 'P' )
    PORT_KEY1(0x08, IP_ACTIVE_LOW, "I       INPUT   AND",     KEYCODE_I,          CODE_NONE, 'I' )
    PORT_KEY1(0x04, IP_ACTIVE_LOW, "RETURN  [Function]",      KEYCODE_ENTER,      CODE_NONE, '\r' )
    PORT_KEY1(0x02, IP_ACTIVE_LOW, "O       LET     OR",      KEYCODE_O,          CODE_NONE, 'O' )
    PORT_KEY1(0x01, IP_ACTIVE_LOW, "Y       ELSE    RIGHT$",  KEYCODE_Y,          CODE_NONE, 'Y' )

    PORT_START /* IN8 KEY ROW 7 */
    PORT_BIT( 0xc0, IP_ACTIVE_LOW, IPT_UNUSED )
    PORT_KEY1(0x20, IP_ACTIVE_LOW, "J       REM     RESET",   KEYCODE_J,          CODE_NONE, 'J' )
    PORT_KEY1(0x10, IP_ACTIVE_LOW, ";       [Rubout]",        KEYCODE_QUOTE,      CODE_NONE, ';' )
    PORT_KEY1(0x08, IP_ACTIVE_LOW, "K       TAB(    PRINT",   KEYCODE_K,          CODE_NONE, 'K' )
	PORT_KEY1(0x04, IP_ACTIVE_LOW, ":       [Inverse]",       KEYCODE_COLON,      CODE_NONE, ':' )
    PORT_KEY1(0x02, IP_ACTIVE_LOW, "L       [Insert]",        KEYCODE_L,          CODE_NONE, 'L' )
    PORT_KEY1(0x01, IP_ACTIVE_LOW, "H       CLS     SET",     KEYCODE_H,          CODE_NONE, 'H' )

    PORT_START /* IN9 EXTRA KEYS */
    PORT_KEY0(0x80, IP_ACTIVE_LOW, "(Inverse)",               KEYCODE_LALT,       CODE_NONE )
    PORT_KEY0(0x40, IP_ACTIVE_LOW, "(Rubout)",                KEYCODE_DEL,        CODE_NONE )
    PORT_KEY0(0x20, IP_ACTIVE_LOW, "(Cursor left)",           KEYCODE_LEFT,       CODE_NONE )
    PORT_KEY0(0x10, IP_ACTIVE_LOW, "(Cursor down)",           KEYCODE_DOWN,       CODE_NONE )
    PORT_KEY0(0x08, IP_ACTIVE_LOW, "(Cursor right)",          KEYCODE_RIGHT,      CODE_NONE )
    PORT_KEY0(0x04, IP_ACTIVE_LOW, "(Backspace)",             KEYCODE_BACKSPACE,  CODE_NONE )
    PORT_KEY0(0x02, IP_ACTIVE_LOW, "(Cursor up)",             KEYCODE_UP,         CODE_NONE )
    PORT_KEY0(0x01, IP_ACTIVE_LOW, "(Insert)",                KEYCODE_INSERT,     CODE_NONE )
