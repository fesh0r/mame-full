...
/* cbm version in kernel at 0xff80 (offset 0x3f80)
   0x80 means pal version */
	 ...
	 /* basic */
	 ROM_LOAD ("318006.01", 0x10000, 0x4000,CRC( 0x74eaae87))

	 /* kernal pal */
	 ROM_LOAD("318004.05",    0x14000, 0x4000,CRC( 0x71c07bd4))
	 ROM_LOAD ("318004.03", 0x14000, 0x4000,CRC( 0x77bab934))

	 /* kernal ntsc */
	 ROM_LOAD ("318005.05", 0x14000, 0x4000,CRC( 0x70295038))
	 ROM_LOAD ("318005.04", 0x14000, 0x4000,CRC( 0x799a633d))

	 /* 3plus1 program */
	 ROM_LOAD ("317053.01", 0x18000, 0x4000,CRC( 0x4fd1d8cb))
	 ROM_LOAD ("317054.01", 0x1c000, 0x4000,CRC( 0x109de2fc))

	 /* same as 109de2fc, but saved from running machine, so
		io area different ! */
	 ROM_LOAD ("3plus1hi.rom", 0x1c000, 0x4000,CRC( 0xaab61387))
	 /* same but lo and hi in one rom */
	 ROM_LOAD ("3plus1.rom", 0x18000, 0x8000,CRC( 0x7d464449))
#endif
...
ROM_START (c16hun)
	 ROM_REGION (0x40000, REGION_CPU1, 0)
	 ROM_LOAD ("318006.01", 0x10000, 0x4000,CRC( 0x74eaae87))
	 ROM_LOAD("hungary.bin",    0x14000, 0x4000,CRC( 0x775f60c5))
ROM_END
...
ROM_START (c364)
	 ROM_REGION (0x40000, REGION_CPU1, 0)
	 ROM_LOAD ("318006.01", 0x10000, 0x4000,CRC( 0x74eaae87))
	 ROM_LOAD ("kern364p.bin", 0x14000, 0x4000,CRC( 0x84fd4f7a))
	 ROM_LOAD ("317053.01", 0x18000, 0x4000,CRC( 0x4fd1d8cb))
	 ROM_LOAD ("317054.01", 0x1c000, 0x4000,CRC( 0x109de2fc))
	 /* at address 0x20000 not so good */
	 ROM_LOAD ("spk3cc4.bin", 0x28000, 0x4000,CRC( 0x5227c2ee))
ROM_END
