...
ROM_START(oric1)
	...
	ROM_LOAD ("basic10.rom", 0x10000, 0x4000,CRC( 0xf18710b4))
	ROM_LOAD_OPTIONAL ("microdis.rom",0x014000, 0x02000,CRC( 0xa9664a9c))
	ROM_LOAD_OPTIONAL ("jasmin.rom", 0x016000, 0x800,CRC( 0x37220e89))
ROM_END

ROM_START(orica)
	ROM_REGION(0x10000+0x04000+0x02000+0x0800,REGION_CPU1,0)
	ROM_LOAD ("basic11b.rom", 0x10000, 0x4000,CRC( 0xc3a92bef))
	ROM_LOAD_OPTIONAL ("microdis.rom",0x014000, 0x02000,CRC( 0xa9664a9c))
	ROM_LOAD_OPTIONAL ("jasmin.rom", 0x016000, 0x800,CRC( 0x37220e89))
ROM_END
...
ROM_START(telstrat)
	...
	ROM_LOAD ("telmatic.rom", 0x010000, 0x02000,CRC( 0x94358dc6))
	ROM_LOAD ("teleass.rom", 0x014000, 0x04000,CRC( 0x68b0fde6))
	ROM_LOAD ("hyperbas.rom", 0x018000, 0x04000,CRC( 0x1d96ab50))
	ROM_LOAD ("telmon24.rom", 0x01c000, 0x04000,CRC( 0xaa727c5d))
ROM_END

ROM_START(prav8d)
    ROM_REGION (0x10000+0x4000+0x0100+0x200,REGION_CPU1,0)
    ROM_LOAD ("pravetzt.rom", 0x10000, 0x4000,CRC( 0x58079502))
	ROM_LOAD_OPTIONAL ("8ddoslo.rom", 0x014000, 0x0100,CRC( 0x0c82f636))
    ROM_LOAD_OPTIONAL ("8ddoshi.rom",0x014100, 0x0200,CRC( 0x66309641))
ROM_END

ROM_START(prav8dd)
    ROM_REGION (0x10000+0x4000+0x0100+0x0200,REGION_CPU1,0)
    ROM_LOAD ("8d.rom", 0x10000, 0x4000,CRC( 0xb48973ef))
    ROM_LOAD_OPTIONAL ("8ddoslo.rom", 0x014000, 0x0100,CRC( 0x0c82f636))
    ROM_LOAD_OPTIONAL ("8ddoshi.rom",0x014100, 0x0200,CRC( 0x66309641))
ROM_END
...
ROM_START(prav8dda)
    ROM_REGION (0x10000+0x4000+0x0100+0x200,REGION_CPU1,0)
    ROM_LOAD ("pravetzd.rom", 0x10000, 0x4000,CRC( 0xf8d23821))
	ROM_LOAD_OPTIONAL ("8ddoslo.rom", 0x014000, 0x0100,CRC( 0x0c82f636))
    ROM_LOAD_OPTIONAL ("8ddoshi.rom",0x014100, 0x0200,CRC( 0x66309641))
ROM_END
