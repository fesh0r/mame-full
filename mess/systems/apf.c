...
ROM_START(apfimag)
	...
	ROM_LOAD("apf_4000.rom",0x010000, 0x00800,CRC( 0x2a331a33))
	ROM_LOAD("basic_68.rom",0x06800, 0x01000,CRC( 0xef049ab8))
	ROM_LOAD("basic_80.rom",0x08000, 0x02000,CRC( 0xa4c69fae))
ROM_END

ROM_START(apfm1000)
	ROM_REGION(0x10000+0x0800,REGION_CPU1,0)
	ROM_LOAD("apf_4000.rom",0x010000, 0x0800,CRC( 0x2a331a33))
ROM_END
