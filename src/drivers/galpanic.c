...
TODO:
- Both games have a vector for IRQ4. The function does nothing in galpanic
  but is more complicated in missw96. However I'm not triggering it, and it
  seems to work fine anyway...
- lots of unknown reads and writes, also in galpanic but particularly in missw96
...
Miss World '96:
---------------
Runs on similar hardware, different sprite system
...
static struct MemoryReadAddress missw96_readmem[] =
{
	{ 0x000000, 0x3fffff, MRA_ROM },
	{ 0x500000, 0x51ffff, galpanic_fgvideoram_r },
	{ 0x520000, 0x53ffff, galpanic_bgvideoram_r },
	...
	{ 0x800004, 0x800005, input_port_2_r },
//	{ 0x800006, 0x800007,  },	??
	{ 0x80000c, 0x80000d, kludge },	/* bits 8-a = timer? palette update code waits for them to be 111 */
	{ 0xc00000, 0xc0ffff, MRA_BANK1 },
	{ 0xf00000, 0xf00001, oki_msb_r },
	{ -1 }  /* end of table */
};

static struct MemoryWriteAddress missw96_writemem[] =
{
	{ 0x000000, 0x3fffff, MWA_ROM },
	{ 0x500000, 0x51ffff, galpanic_fgvideoram_w, &galpanic_fgvideoram, &galpanic_fgvideoram_size },
	{ 0x520000, 0x53ffff, galpanic_bgvideoram_w, &galpanic_bgvideoram },	/* + work RAM */
	...
	{ 0xc00000, 0xc0ffff, MWA_BANK1 },
	{ 0xf00000, 0xf00001, oki_msb_w },
	{ -1 }  /* end of table */
};
	...
	{
		{
			CPU_M68000,
			8000000,	/* 8 MHz ??? */
			galpanic_readmem,galpanic_writemem,0,0,
...
static struct MachineDriver machine_driver_missw96 =
{
	/* basic machine hardware */
	{
		{
			CPU_M68000,
			10000000,	/* 10 MHz ??? */
			missw96_readmem,missw96_writemem,0,0,
			galpanic_interrupt,2
		}
	},
	60, DEFAULT_60HZ_VBLANK_DURATION,	/* frames per second, vblank duration */
...
GAMEX( 1996, missw96,  0, missw96,  missw96,  0, ROT0_16BIT,  "Comad", "Miss World '96 Nude", GAME_NO_COCKTAIL )
