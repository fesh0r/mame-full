...
TODO:
	- Correct colours
	- Add sound whit a corret rom
	- For what is used the other prom?
...
WRITE_HANDLER( enigma2_videoram_w )
{
	if (videoram[offset] != data)
	{
		...
		{
			if( data & 0x80 )
				plot_pixel(tmpbitmap, x, 255 - y, Machine->pens[7]);
			
//to get correct colours it should be something similar
//				plot_pixel(tmpbitmap, x, 255 - y, memory_region(REGION_PROMS)[(y >> 3 << 5) | ((col/*+i*/) >> 3)] & 0x07);
...
//	PORT_DIPSETTING(    0x1c, "?" )
	...
	PORT_DIPSETTING(    0x00, DEF_STR( Off ) )
	PORT_DIPSETTING(    0x20, DEF_STR( On ) )
	PORT_DIPNAME( 0x40, 0x40, "Number of invaders" ) // Number of invaders 1st screen 1st round
...
ROM_START( enigma2 )
	...
	ROM_LOAD( "32_en5.bin",   0x4000, 0x0800, CRC(098ac15b) SHA1(cce28a2540a9eabb473391fff92895129ae41751) ) 
	ROM_LOAD( "42_en6.bin",   0x4800, 0x0800, CRC(240a9d4b) SHA1(ca1c69fafec0471141ce1254ddfaef54fecfcbf0) ) 
	...
	// taken from the other set, but it should be the same
	ROM_REGION( 0x800, REGION_PROMS, 0 ) 
	ROM_LOAD( "7.11f",        0x0000, 0x0800, CRC(409b5aad) SHA1(1b774a70f725637458ed68df9ed42476291b0e43) ) //FIXED BITS (00000xxx)
ROM_END
...
ROM_START( enigma2a )
	...
	ROM_LOAD( "32_en5.bin",   0x4000, 0x0800, CRC(098ac15b) SHA1(cce28a2540a9eabb473391fff92895129ae41751) ) // 5.11d
	ROM_LOAD( "42_en6.bin",   0x4800, 0x0800, CRC(240a9d4b) SHA1(ca1c69fafec0471141ce1254ddfaef54fecfcbf0) ) // 6.13d
	...
	ROM_LOAD( "8.13f",        0x0000, 0x0800, CRC(e9cb116d) SHA1(41da4f46c5614ec3345c233467ebad022c6b0bf5) ) //FIXED BITS (00000xxx)
ROM_END

GAMEX( 1984, enigma2,  0,		enigma2, enigma2,  0, ROT90, "Zilec Electronics", "Enigma 2", GAME_NO_SOUND | GAME_WRONG_COLORS )
GAMEX( 1981, enigma2a, enigma2, enigma2, enigma2a, 0, ROT90, "Game Plan (Zilec Electronics license)", "Enigma 2 (old version)", GAME_NO_SOUND | GAME_WRONG_COLORS )
