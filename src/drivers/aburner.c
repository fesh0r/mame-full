...
INPUT_PORTS_START( aburner )
	...
	PORT_START
	PORT_BIT( 0xff, 0x7f, IPT_AD_STICK_X ) PORT_MINMAX(0x00,0xff) PORT_SENSITIVITY(100) PORT_KEYDELTA(4) PORT_CENTER

	PORT_START
	PORT_BIT( 0xff, 0x7f, IPT_AD_STICK_Y ) PORT_MINMAX(0x00,0xff) PORT_SENSITIVITY(100) PORT_KEYDELTA(4) PORT_CENTER PORT_REVERSE

	PORT_START /* throttle (old player2 hack removed, now uses Z axis) */
	PORT_BIT( 0xff, 0x80, IPT_AD_STICK_Z ) PORT_MINMAX(0x00,0xff) PORT_SENSITIVITY(100) PORT_KEYDELTA(79) PORT_CENTER
INPUT_PORTS_END
...
INPUT_PORTS_START( thndrbld )
	...
	PORT_START
	PORT_BIT( 0xff, 0x7f, IPT_AD_STICK_X ) PORT_MINMAX(0x00,0xff) PORT_SENSITIVITY(100) PORT_KEYDELTA(4) PORT_CENTER PORT_REVERSE

	PORT_START /* throttle (old player2 hack removed, now uses Z axis) */
	PORT_BIT( 0xff, 0x80, IPT_AD_STICK_Z ) PORT_MINMAX(0x00,0xff) PORT_SENSITIVITY(100) PORT_KEYDELTA(79) PORT_CENTER

	PORT_START
	PORT_BIT( 0xff, 0x7f, IPT_AD_STICK_Y ) PORT_MINMAX(0x00,0xff) PORT_SENSITIVITY(100) PORT_KEYDELTA(4) PORT_CENTER
INPUT_PORTS_END
...
static ADDRESS_MAP_START( aburner_readmem2, ADDRESS_SPACE_PROGRAM, 16 )
	...
	AM_RANGE(0x200000, 0x27ffff) AM_READ(MRA16_ROM) /* mirror */
	AM_RANGE(0x29c000, 0x2a3fff) AM_READ(SYS16_MRA16_WORKINGRAM2_SHARE) /* mirror */
ADDRESS_MAP_END
...
static ADDRESS_MAP_START( aburner_writemem2, ADDRESS_SPACE_PROGRAM, 16 )
	...
	AM_RANGE(0x29c000, 0x2a3fff) AM_WRITE(SYS16_MWA16_WORKINGRAM2_SHARE) /* mirror */
ADDRESS_MAP_END
