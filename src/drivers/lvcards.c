...
static ADDRESS_MAP_START( lcmap , ADDRESS_SPACE_PROGRAM, 8  )
	...
	AM_RANGE(0xa001, 0xa001) AM_READWRITE(input_port_1_r,MWA8_NOP ) /* outputs */
	AM_RANGE(0xa002, 0xa002) AM_READWRITE(input_port_2_r,MWA8_NOP ) /* outputs */
	AM_RANGE(0xc000, 0xdfff) AM_ROM
ADDRESS_MAP_END

static ADDRESS_MAP_START( port, ADDRESS_SPACE_IO, 8 )
...
INPUT_PORTS_START( lvcards )
	PORT_START
	PORT_BIT(0x01, IP_ACTIVE_LOW, 0 ) PORT_NAME("Analyzer") PORT_CODE(KEYCODE_0)
	PORT_BIT( 0x02, IP_ACTIVE_LOW, IPT_SERVICE1 )
	PORT_BIT( 0x04, IP_ACTIVE_LOW, IPT_UNKNOWN )
	PORT_BIT( 0x08, IP_ACTIVE_LOW, IPT_UNKNOWN )
	PORT_BIT( 0x10, IP_ACTIVE_LOW, IPT_UNKNOWN )
	PORT_BIT( 0x20, IP_ACTIVE_LOW, IPT_UNKNOWN )
	PORT_BIT(0x40, IP_ACTIVE_LOW, 0 ) PORT_NAME("Red") PORT_CODE(KEYCODE_N)
	PORT_BIT(0x80, IP_ACTIVE_LOW, 0 ) PORT_NAME("Black") PORT_CODE(KEYCODE_M)


	PORT_START
	PORT_BIT(0x01, IP_ACTIVE_LOW, 0 ) PORT_NAME("Hold 1") PORT_CODE(KEYCODE_Z)
	PORT_BIT(0x02, IP_ACTIVE_LOW, 0 ) PORT_NAME("Hold 2") PORT_CODE(KEYCODE_X)
	PORT_BIT(0x04, IP_ACTIVE_LOW, 0 ) PORT_NAME("Hold 3") PORT_CODE(KEYCODE_C)
	PORT_BIT(0x08, IP_ACTIVE_LOW, 0 ) PORT_NAME("Hold 4") PORT_CODE(KEYCODE_V)
	PORT_BIT(0x10, IP_ACTIVE_LOW, 0 ) PORT_NAME("Hold 5") PORT_CODE(KEYCODE_B)
	PORT_BIT(0x20, IP_ACTIVE_LOW, 0 ) PORT_NAME("Deal / Double") PORT_CODE(KEYCODE_LALT)
	PORT_BIT(0x40, IP_ACTIVE_LOW, 0 ) PORT_NAME("Bet") PORT_CODE(KEYCODE_LCONTROL)
	...
	PORT_BIT( 0x08, IP_ACTIVE_LOW, IPT_UNKNOWN )
	PORT_BIT( 0x04, IP_ACTIVE_LOW, IPT_UNKNOWN )
	PORT_BIT(0x02, IP_ACTIVE_LOW, 0 ) PORT_NAME("Cancel / Take Score") PORT_CODE(KEYCODE_SPACE)
...
static MACHINE_DRIVER_START( lvcards )
	...
	MDRV_CPU_PROGRAM_MAP(lcmap,0)
	MDRV_CPU_IO_MAP(port,0)
