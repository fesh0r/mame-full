...
{
	static int count = 0;
	usrintf_showmessage("WaveRAM writes = %X", ++count);
}
			}
			break;
		}
...
//9d0000 -- page select?
	...
	AM_RANGE(0x9f0000, 0x9f7fff) AM_READWRITE(cmos_r, cmos_w) AM_BASE((data32_t **)&generic_nvram) AM_SIZE(&generic_nvram_size)
...
GAMEX( 1999, crusnexo, 0,     midzeus, crusnexo, crusnexo, ROT0, "Midway", "Cruisin' Exotica", GAME_NOT_WORKING | GAME_NO_SOUND )
