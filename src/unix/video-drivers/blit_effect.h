...
{ \
  ...
  for (y = visual.min_y; y <= visual.max_y; line_dest+=heightscale*DEST_WIDTH, y++) { \
...
{ \
  for (;line_src < line_end; line_dest+=heightscale*DEST_WIDTH, line_src+=src_width) { \
    FUNC; \
  } \
}  
...
{ \
  ...
  \
  for (y = visual.min_y; y <= visual.max_y; line_dest+=2*DEST_WIDTH, y++) { \
    /* shift lines up */ \
    tmp = rotate_dbbuf0; \
...
{ \
  ...
  for (;line_src < line_end; line_dest+=2*DEST_WIDTH, line_src+=src_width) { \
...
#	endif
	...
	for (y = visual.min_y; y <= visual.max_y; y++, line_dest+=2*DEST_WIDTH) {
