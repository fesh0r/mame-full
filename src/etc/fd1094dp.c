...
static void print_global_key(int gkey1,int gkey2,int gkey3)
{
	printf("global key: ");
	print_binary(gkey1);
	printf(" ");
	...
	if (BIT(gkey1,7) == 0) { printf("gkey[1],7 == 0! Contact MAMEDEV!\n"); exit(0); }
	if (BIT(gkey1,5) == 0) { printf("gkey[1],5 == 0! Contact MAMEDEV!\n"); exit(0); }
	if (BIT(gkey1,3) == 0) { printf("gkey[1],3 == 0! Contact MAMEDEV!\n"); exit(0); }
	if (BIT(gkey1,6) != BIT(gkey1,4)) { printf("gkey[1],6 != gkey[1],4! Contact MAMEDEV!\n"); exit(0); }
	...
	if (BIT(gkey3,5) == 0) { printf("gkey[3],5 == 0! Contact MAMEDEV!\n"); exit(0); }
}
...
void find_global_key(void)
{
				...
				if (check_key(gkey1,BIT(gkey1,6),gkey1,gkey2,gkey3))
				{
					...
					if (check_key(gkey2,BIT(gkey2,6),gkey1,gkey2,gkey3))
					{
						...
						if (check_key(gkey3,BIT(gkey3,6),gkey1,gkey2,gkey3))
						{
							...
							global_key2 = gkey2;
							global_key3 = gkey3;
							print_global_key(gkey1,gkey2,gkey3);
							found++;
						}
					}
				}
	...
	if (found > 1)
	{
		...
		printf("More than one global key found. Contact MAMEDEV.\n");
		exit(0);
	}
		...
		for (gkey1 = 0;gkey1 < 0x100;gkey1++)
		{
			...
			for (gkey2 = 0;gkey2 < 0x100;gkey2++)
			{
				for (gkey3 = 0;gkey3 < 0x100;gkey3++)
				{
					...
					for (key1 = gkey1 & 0x0f;key1 < 0x100;key1 += 0x10)
					{
						if (check_key(key1,BIT(key1,6),gkey1,gkey2,gkey3))
						{
							for (key2 = gkey2 & 0x1f;key2 < 0x100;key2 += 0x20)
							{
								load_table(4);
								if (check_key(key2,BIT(key2,6),gkey1,gkey2,gkey3))
								{
