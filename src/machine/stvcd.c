...
UINT8 * 	CD_info_ptr;			// info transfer buffer pointer
...
UINT8	cdb_sat_file_info[254 * 12];	// current file info
UINT8 cdb_sat_subq[5 * 2];		// current subcode q
UINT8 cdb_sat_subrw[12 * 2];		// current subcode r~w
UINT8 CD_sat_subq[5 * 2];			// current subcode q
UINT8 CD_sat_subrw[12 * 2];		// current subcode r~w
toc_t	CD_toc;			// disc toc
file_t	CD_file[CDB_FID_NUM];	// file table (directory table)
UINT32	CD_file_num;		// total file infos stored
UINT8	CD_sat_toc[408];	// current cdrom toc
...
UINT32 iso_read_sector(UINT32 mode, UINT32 fad, UINT8 * dst){
	...
	logerror("mode = %i fad = %i ", mode, fad);
