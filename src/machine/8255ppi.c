...
data_t ppi8255_r(int which, offs_t offset)
{
	...
	if (which > intf->num)
	{
		logerror("Attempting to read an unmapped 8255 chip (%d)\n", which);
		return 0;
	}
...
void ppi8255_w(int which, offs_t offset, data_t data)
{
	...
	if (which > intf->num)
	{
		logerror("Attempting to write an unmapped 8255 chip (%d)\n", which);
		return;
	}

	if (offset > 3)
	{
		logerror("Attempting to write an invalid 8255 register (%d)\n", offset);
		return;
	}
	...
	case 3:							/* Control word */
		if (data & 0x80)
		{
			...
			{
				logerror("8255 chip %d: Setting an unsupported mode ($%02X)!\n", which, data);
			}
...
data_t ppi8255_peek( int which, offs_t offset )
{
	...
	if (offset > 2)
	{
		logerror("Attempting to peek an invalid 8255 port (%d)\n", offset);
		return 0xff;
	}
