...
static void Mapper18_Write (int offset, int data)
{
	...
	switch (offset & 0x7003)
	{
		...
		case 0x5001:
			...
			vrom_bank[6] &= ((CHR_Rom << 1) - 1);
			nes_vram[6] = vrom_bank[6] * 64;
			if (errorlog) fprintf (errorlog, "     Mapper 18 vrom ($1800 high 4) value: %02x\n", data);
			break;
		case 0x5002:
			/* Switch 1k vrom at $1c00 - low 4 bits */
			vrom_bank[7] = (vrom_bank[7] & 0xf0) | (data & 0x0f);
			vrom_bank[7] &= ((CHR_Rom << 1) - 1);
		...
		case 0x5003:
			...
			vrom_bank[7] &= ((CHR_Rom << 1) - 1);
			nes_vram[7] = vrom_bank[7] * 64;
			if (errorlog) fprintf (errorlog, "     Mapper 18 vrom ($1c00 high 4) value: %02x\n", data);
			break;
...
#endif
		default:
			if (errorlog) fprintf (errorlog, "Mapper 33 addr: %04x value: %02x\n", offset + 0x8000, data);
			break;
	}
}
