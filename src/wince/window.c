...
#else
INLINE void erase_outer_rect(RECT *outer, RECT *inner, HDC dc, void *dummy)
#endif
{
...
int win32_init_window(void)
{
...
#ifdef UNICODE
	#ifndef MESS
	wsprintf(title, TEXT("MAME: %S [%S]"), Machine->gamedrv->description, Machine->gamedrv->name);
	#else
	wsprintf(title, TEXT("MESS: %S [%S]"), Machine->gamedrv->description, Machine->gamedrv->name);
	#endif
#else
	#ifndef MESS
	...
	video_window = CreateWindowEx(window_mode ? WINDOW_STYLE_EX : FULLSCREEN_STYLE_EX,
			TEXT("MAME"), title, window_mode ? WINDOW_STYLE : FULLSCREEN_STYLE,
...
void toggle_full_screen(void)
{
	...
	{
#ifdef UNDER_CE
		primary_desc.dwWidth = GetSystemMetrics(SM_CXSCREEN);
		primary_desc.dwHeight = GetSystemMetrics(SM_CYSCREEN);
#else // !UNDER_CE
		DEVMODE device_data;
...
static void dib_draw_window(HDC dc, struct osd_bitmap *bitmap, int update)
{
	...
	video_dib_info->bmiHeader.biWidth = ((visible_width * xmult) + 3) & ~3;
	video_dib_info->bmiHeader.biHeight = -visible_height * ymult;
	video_dib_info->bmiHeader.biBitCount = depth;

	// create a dibsection
	hBitmap = CreateDIBSection(dc, video_dib_info, DIB_RGB_COLORS, (void **) &pvBits, NULL, 0);
	if (!hBitmap)
		return;

	// copy into dibsection
	for (i = 0; i < visible_height; i++)
	{
		memcpy(pvBits, bitmap->line[i], visible_width * 8 / depth);
