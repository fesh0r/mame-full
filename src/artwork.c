...
struct artwork_ *artwork_backdrop = NULL;

/* the overlay instance */
struct artwork_ *artwork_overlay = NULL;
 ...
 *********************************************************************/
void backdrop_set_palette(struct artwork_ *a, UINT8 *palette)
{
	int i;
 ...
 *********************************************************************/
static unsigned int *transparency_hist (struct artwork_ *a, int num_shades)
{
	int i, j;
	unsigned int *hist;
 ...
 *********************************************************************/
static void allocate_artwork_mem (int width, int height, struct artwork_ **a)
{
	if (Machine->orientation & ORIENTATION_SWAP_XY)
	{
		int temp;
	...
	*a = (struct artwork_ *)malloc(sizeof(struct artwork_));
	if (*a == 0)
	{
...
static void artwork_load_size_common(const char *filename, unsigned int start_pen, unsigned int max_pens,
					   				 int width, int height, struct artwork_ **a)
{
...
void artwork_load_size(struct artwork_ **a, const char *filename, unsigned int start_pen, unsigned int max_pens,
...
static int artwork_newpen (struct artwork_ *a, int r, int g, int b, int alpha)
{
	int pen;
...
static void merge_cmy(struct artwork_ *a, struct osd_bitmap *source, struct osd_bitmap *source_alpha,int sx, int sy)
{
