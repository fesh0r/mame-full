...
static struct osd_bitmap *dlybitmap;
...
int m6845_vh_start(void)
{
    ...
    dlybitmap = osd_new_bitmap(Machine->drv->screen_width,Machine->drv->screen_height,Machine->scrbitmap->depth);
	if (!dlybitmap)
		return 1;
...
***************************************************************************/
int m6845_get_register(int indx)
{
	switch (indx)
	{
		case 0:
...
***************************************************************************/
void m6845_vh_screenrefresh(struct osd_bitmap * bitmap, int full_refresh)
{
int i, address, offset, cursor, size, code, x, y, color0;
struct rectangle r;
byte *sp, *dp;
