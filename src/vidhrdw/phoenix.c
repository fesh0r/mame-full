...
static void get_fg_tile_info(int tile_index)
{
	...
	tile_info.priority = (tile_index & 0x1f) ? 0 : 1;	/* first row (column) is opaque */
}
...
void phoenix_vh_screenrefresh(struct osd_bitmap *bitmap,int full_refresh)
{
	...
	tilemap_draw(bitmap,fg_tilemap,TILE_IGNORE_TRANSPARENCY | 1);	/* first opaque row */
	tilemap_draw(bitmap,fg_tilemap,0);								/* the transparent rows */
}
