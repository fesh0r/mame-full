...
/* 27/06/98 - Juergen Buchmueller - implemented missing POKEY registers.	 */
...
/* that is repeatedly called after Div_n_max[chan]/intf->clock time is gone. */
/* This function checks for enabled timer IRQs in IRQEN and sets the		 */
...
/* minimum update step for POKEY (226usec @ 44100Hz) */
#define MIN_SLICE	10
...
static signed char *sample_buffer;
...
void  *TIMER[MAXPOKEYS][3];         /* timers returned by timer.c */
uint8 KBCODE[MAXPOKEYS];			/* KBCODE (R/D209) */
uint8 SERIN[MAXPOKEYS];             /* SERIN (R/D20D) */
uint8 SEROUT[MAXPOKEYS];			/* SEROUT (W/D20D) */
uint8 IRQST[MAXPOKEYS];             /* IRQST (R/D20E) */
uint8 IRQEN[MAXPOKEYS]; 			/* IRQEN (W/D20E) */
uint8 SKSTAT[MAXPOKEYS];			/* SKSTAT (R/D20F) */
uint8 SKCTL[MAXPOKEYS]; 			/* SKCTL (W/D20F) */
...
/* The random number for each pokey */
static uint8 RANDOM[MAXPOKEYS];
/* The absolute time when the random number for a chip was last read */
static double RNGTIME[MAXPOKEYS];   
/* The current offset into the rand17 table for each chip */
static uint32 RNGOFFS[MAXPOKEYS];
...
/*                                                                           */
/*****************************************************************************/
static void Poly_init(uint8 * p, uint32 size, uint8 a, uint8 b, uint32 c)
{
uint32 i, x = 0;
    for (i = 0; i < size; i++)
	{
...
static void Rand_init(uint8 * p, uint32 size, uint8 a, uint8 b, uint32 c)
{
uint32 i, x = 0;
    for (i = 0; i < size; i++)
	{
...
/*    _disable();*/		/* RSF - removed for portability 31-MAR-97 */
		...
		case STIMER_C:
			...
			if (AUDCTL[chip] & CH1_CH2)
			{
				if (Div_n_tmr[CHAN2 + chip_offs] > MIN_TIMER)
                {
					...
					TIMER[chip][TIMER2] =
						timer_pulse(1.0 * Div_n_tmr[CHAN2 + chip_offs] / intf->clock,
			...
			else
			{
				if (Div_n_tmr[CHAN1 + chip_offs] > MIN_TIMER)
                {
					...
					TIMER[chip][TIMER1] =
						timer_pulse(1.0 * Div_n_tmr[CHAN1 + chip_offs] / intf->clock,
            ...
            if (AUDCTL[chip] & CH3_CH4)
			{
				...
				if (AUDC[CHAN4 + chip_offs] & 0xf0)
				{
					if (Div_n_tmr[CHAN4 + chip_offs] > MIN_TIMER)
					{
						...
						TIMER[chip][TIMER4] =
							timer_pulse(1.0 * Div_n_tmr[CHAN4 + chip_offs] / intf->clock,
    ...
    if (chan_mask & (1 << CHAN1))
    {
        ...
        if (new_val != Div_n_tmr[CHAN1 + chip_offs])
		{
			Div_n_tmr[CHAN1 + chip_offs] = new_val;
			if (TIMER[chip][TIMER1] && new_val > MIN_TIMER)
				timer_reset(TIMER[chip][TIMER1], 1.0 * new_val / intf->clock);
		}
...
#endif
        /* timer 'div by n' changed ? */
        if (new_val != Div_n_tmr[CHAN2 + chip_offs])
		{
			Div_n_tmr[CHAN2 + chip_offs] = new_val;
			if (TIMER[chip][TIMER2] && new_val > MIN_TIMER)
				timer_reset(TIMER[chip][TIMER2], 1.0 * new_val / intf->clock);
        }
...
#endif
        /* timer 'div by n' changed ? */
        if (new_val != Div_n_tmr[CHAN4 + chip_offs])
		{
			Div_n_tmr[CHAN4 + chip_offs] = new_val;
			if (TIMER[chip][TIMER4] && new_val > MIN_TIMER)
				timer_reset(TIMER[chip][TIMER4], 1.0 * new_val / intf->clock);
        }
...
void Pokey_process (register signed char *buffer, register uint16 n)
{
	...
	REGISTER int32 cur_val; 	/* then we have to count as 16-bit */
...
int pokey_sh_start (struct POKEYinterface *interface)
{
	...
	if ((sample_buffer = malloc(buffer_len)) == 0)
        return 1;
	memset(sample_buffer,0,buffer_len);
...
static void update_pokeys(void)
{
    ...
    newpos = cpu_scalebyfcount(buffer_len); 

    if (newpos - sample_pos < MIN_SLICE)
        return;
	Pokey_process (sample_buffer + sample_pos, newpos - sample_pos);
        ...
        case RANDOM_C:
			{
				...
				{
                double delta_t = time_now - RNGTIME[chip];
				int32 shifted = delta_t * intf->clock;
					/* find new position in rand17 table */
					RNGOFFS[chip] = abs(RNGOFFS[chip] + shifted) % POLY17_SIZE;
					/* and get the random value */
					RANDOM[chip] = rand17[RNGOFFS[chip]];
#if VERBOSE_RANDOM
					if (errorlog) fprintf (errorlog, "POKEY #%d rand17 shifted %7u times: $%02x\n", chip, shifted, RANDOM[chip]);
#endif
				}
				else
				{
...
{
	timer_set(1.0 * after / intf->clock, 0, Pokey_SerinReady);
}

void pokey2_serin_ready(int after)
{
	timer_set(1.0 * after / intf->clock, 1, Pokey_SerinReady);
}

void pokey3_serin_ready(int after)
{
	timer_set(1.0 * after / intf->clock, 2, Pokey_SerinReady);
}

void pokey4_serin_ready(int after)
{
	timer_set(1.0 * after / intf->clock, 3, Pokey_SerinReady);
}
...
void pokey_break_w(int chip, int shift)
{
    ...
    if (IRQEN[chip] & IRQ_BREAK)
    {
		...
		if (intf->interrupt_cb)
			(*intf->interrupt_cb)(IRQ_BREAK);
    }
}
...
void pokey_sh_update (void)
{
    ...
    if (sample_pos < buffer_len)
		Pokey_process (sample_buffer + sample_pos, buffer_len - sample_pos);
    sample_pos = 0;

	osd_play_streamed_sample(channel,(signed char *)sample_buffer,buffer_len,emulation_rate,intf->volume);
}
