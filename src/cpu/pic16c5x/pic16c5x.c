...
#define PSA_FLAG	0x04	/* PSA		Prescaler Assignment bit */
#define PS_REG		0x03	/* PS		Prescaler Rate select */

#define T0SE	(R.OPTION & T0SE_FLAG)
#define T0ES	(R.OPTION & T0ES_FLAG)
...
#define WDTE_FLAG	0x04	/* WDTE		Watch Timer enable */
#define FOSC_FLAG	0x03	/* FOSC		Oscillator source select */

#define WDTE	(R.CONFIG & WDTE_FLAG)
#define FOSC	(R.CONFIG & FOSC_FLAG)
///Page 44  Section 9.1 See the bottom about another doco required for
///this Config register  Doc No DS30190
...
INLINE UINT8 GET_REGFILE(offs_t addr)	/* Read from internal memory */
{
	...
	{
		addr |= (R.picRAM[(R.FSR)] & 0x60);
	}
	if ((addr & 0x30) == 0x20) addr &= 0x0f;

	switch(addr)
	{
		case 00:	if ((R.picRAM[(R.FSR)]) == 0) { data = 0; break; }
					data = R.picRAM[(R.FSR)]; break;	/* Indirect address */
		case 04:	data = R.FSR | 0xe0; break;
		case 05:	data = P_IN(0);
					data &= R.TRISA;
...
INLINE void STORE_REGFILE(offs_t addr, UINT8 data)	/* Write to internal memory */
{
	if ((picmodel == 0x16C57) || (picmodel == 0x16C58))
	{
		addr |= (R.picRAM[((R.FSR) & 0x7f)] & 0x60);
	}
	if ((addr & 0x30) == 0x20) addr &= 0x0f;

	switch(addr)
	{
		case 00:	if ((R.picRAM[(R.FSR)]) == 0) break;
					R.picRAM[(R.FSR)] = data; break;	/* Indirect address */
		case 01:	delay_timer = 2;		/* Timer starts after next two instructions */
					if (PSA == 0) R.OPTION &= ~PS_REG;	/* Must clear the Prescaler */
					R.TMR0 = data; break;
		case 02:	R.PCL = data;
					R.PC = ((R.STATUS & PA_REG) >> 4) | data; break;
		case 03:	R.STATUS = data & 0xe0; break;
		case 04:	R.FSR = data; break;
...
static unsigned cycles_000_other[16]=
{
/*00*/	1*CLK, 0*CLK, 1*CLK, 1*CLK, 0*CLK, 1*CLK, 1*CLK, 1*CLK, 0*CLK, 0*CLK, 0*CLK, 0*CLK, 0*CLK, 0*CLK, 0*CLK, 0*CLK
};
...
void pic16C5x_init(void)
{
	...
	state_save_register_UINT8("pic16C5x", cpu, "WDT", &R.WDT, 1);
	...
	state_save_register_UINT32("pic16C5x", cpu, "Prescaler", &R.prescaler, 1);
...
void pic16C5x_reset_regs(void)
{
	...
	R.CONFIG = 0x00;	/* Actually this is hard programmed on the chip */
...
static void pic16C5x_update_watchdog(void)
{
	if (PSA) {
		R.prescaler++;
...
unsigned pic16C5x_get_reg(int regnum)
{
	switch (regnum)
	{
		...
		case PIC16C5x_FSR:  return R.FSR;
...
void pic16C5x_set_reg(int regnum, unsigned val)
{
	switch (regnum)
	{
		...
		case PIC16C5x_FSR:  R.FSR    = val; break;
...
static UINT8 pic16C5x_3p_reg_layout[] = {
	...
	PIC16C5x_W,   PIC16C5x_TMR0, PIC16C5x_PRTA, PIC16C5x_PRTB, PIC16C5x_PRTC, -1,
	PIC16C5x_ALU, PIC16C5x_WDT,  PIC16C5x_TRSA, PIC16C5x_TRSB, PIC16C5x_TRSC, 0
};
#endif
#if (HAS_PIC16C54 || HAS_PIC16C56 || HAS_PIC16C58)
static UINT8 pic16C5x_2p_reg_layout[] = {
	PIC16C5x_PC,  PIC16C5x_STK0, PIC16C5x_STK1, PIC16C5x_STR,  PIC16C5x_OPT, -1,
	PIC16C5x_W,   PIC16C5x_TMR0, PIC16C5x_PRTA, PIC16C5x_PRTB, -1,
	PIC16C5x_ALU, PIC16C5x_WDT,  PIC16C5x_TRSA, PIC16C5x_TRSB, 0
};
#endif
...
const char *pic16C5x_info(void *context, int regnum)
{
	...
	switch (regnum)
	{
		...
		case CPU_INFO_REG+PIC16C5x_WDT:  sprintf(buffer[which], "WDT:%02X",  r->WDT); break;
		...
		case CPU_INFO_FLAGS:
			sprintf(buffer[which], "%01x%c%c%c%c%c  %c%c%c%02x",
				(r->STATUS & 0xe0) >> 5,
		...
		case CPU_INFO_VERSION: return "1.00";
...
unsigned pic16C54_get_reg(int regnum) { pic16C5x_get_reg(regnum); }
...
unsigned pic16C56_get_reg(int regnum) { pic16C5x_get_reg(regnum); }
...
unsigned pic16C58_get_reg(int regnum) { pic16C5x_get_reg(regnum); }
