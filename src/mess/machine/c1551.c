...
void cbm_drive_close(void)
{
  ...
  for (i=0;i<sizeof(cbm_drive)/sizeof(CBM_Drive); i++) {
    ...
    if (cbm_drive[i].drive==D64_IMAGE) {
      if (cbm_drive[i].d.d64.image) free(cbm_drive[i].d.d64.image);
    }
    cbm_drive[i].drive=0;
  }
}
...
{
  int i;
  if (drive->interface==interface) return;
  ...
  if (drive->interface==IEC) {
    ...
    if (drive==cbm_drive) drive->i.serial.device=8;
    else drive->i.serial.device=9;
...
static int d64_open(CBM_Drive *drive, const char *imagename)
{
  ...
  if (errorlog) fprintf(errorlog, "floppy image %s loaded\n", imagename);
...
void c1551_0_write_data(int data) { c1551_write_data(cbm_drive,data); }
int c1551_0_read_data(void) { return c1551_read_data(cbm_drive); }
void c1551_0_write_handshake(int data) 
{ c1551_write_handshake(cbm_drive,data); }
int c1551_0_read_handshake(void) { return c1551_read_handshake(cbm_drive); }
int c1551_0_read_status(void) { return c1551_read_status(cbm_drive); }

void c1551_1_write_data(int data) { c1551_write_data(cbm_drive+1,data); }
int c1551_1_read_data(void) { return c1551_read_data(cbm_drive+1); }
void c1551_1_write_handshake(int data) 
{ c1551_write_handshake(cbm_drive+1,data); }
int c1551_1_read_handshake(void) { return c1551_read_handshake(cbm_drive+1); }
int c1551_1_read_status(void) { return c1551_read_status(cbm_drive+1); }
...
void cbm_serial_data_write(int level)
{
  ...
  /* update line */
  for (i=0;i<cbm_serial.count;i++) cbm_serial.data[0]&=cbm_serial.data[i+2];
  /* inform drives */
  for (i=0;i<cbm_serial.count;i++)
...
void cbm_serial_clock_write(int level)
{
  ...
  /* update line */
  for (i=0;i<cbm_serial.count;i++) cbm_serial.clock[0]&=cbm_serial.clock[i+2];
  /* inform drives */
  for (i=0;i<cbm_serial.count;i++)
...
void cbm_serial_atn_write(int level)
{
  ...
  /* update line */
  for (i=0;i<cbm_serial.count;i++) cbm_serial.atn[0]&=cbm_serial.atn[i+2];
  /* inform drives */
  for (i=0;i<cbm_serial.count;i++)
