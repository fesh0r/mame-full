...
int	jupiter_load_ace (void);
int	jupiter_load_tap (void);
...
if ((pc == 0x059d)) {
  ...
  if ((jupiter_data_type == JUPITER_ACE)) {
    ...
    tmpword = cpu_readmem16 (0x3c3b) + cpu_readmem16 (0x3c3c) * 256
						    + jupiter_tape.hdr_len;
...
if (errorlog) fprintf (errorlog, "jupiter_init\r\n");
...
int	jupiter_rom_load (void)

{

if (strlen (rom_name[0])) {
  if (!strcmp (strrchr (rom_name[0], '.'), ".ace"))
  						return (jupiter_load_ace ());
  else {
    if (errorlog) fprintf (errorlog, "Only .ace roms supported.\r\n");
    return (1);
  }
} else if (cassette_name[0][0]) return (jupiter_load_tap ());
...
jupiter_index = 0;
if ((file = osd_fopen (Machine->gamedrv->name, rom_name[0],
				       OSD_FILETYPE_IMAGE_RW, 0))) {
  if ((jupiter_data = malloc (0x6000))) {
	...
	case 0x01:
	  ...
	  break;
	case 0x02:
	  if (errorlog) fprintf (errorlog, "Sequence 0xED 0x02 found in .ace file\r\n");
	  break;
	default:
      ...
      } else jupiter_data[jupiter_index++] = jupiter_byte;
    }
  }
  osd_fclose (file);
}
if (!done) {
  if (errorlog) fprintf (errorlog, "file not loaded\r\n");
  if (jupiter_data) {
    free (jupiter_data);
    jupiter_data = NULL;
  }
...
if (errorlog) fprintf (errorlog, "Decoded %d bytes.\r\n", jupiter_index);
...
if ((file = osd_fopen (Machine->gamedrv->name, cassette_name[0],
				       OSD_FILETYPE_IMAGE_RW, 0))) {
  ...
  if (hdr_len > 0x19) for (loop = 0x19; loop < hdr_len; loop++)
  						osd_fread (file, &inpbyt, 1);
