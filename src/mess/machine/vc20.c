...
int vc20_rom_init(int id, const char *name)
{
  romnames[id]=name;
  return 0;
}
...
int vc20_rom_load(int id, const char *name)
{
  ...
  if(!vc20_rom_id(name, Machine->gamedrv->name)) return 1;
...
int vc20_frame_interrupt (void)
{
  ...
  if (!quickload && QUICKLOAD) cbm_quick_open(0,vc20_memory);
  quickload=QUICKLOAD;
