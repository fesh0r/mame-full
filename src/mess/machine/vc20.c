...
CBM_Drive vc20_drive8={0}, vc20_drive9={0};
...
static int vc20_via0_read_porta(int offset)
{
  ...
  if( !serial_clock || !cbm_serial_clock_read()) value&=~1;
  if( !serial_data || !cbm_serial_data_read()) value&=~2;
...
void vc20_init_machine(void)
{
  ...
  if ((j==0)&&SERIAL8ON) vc1541_fs_open(8,&vc20_drive8);
  if ((j<=1)&&SERIAL9ON) vc1541_fs_open(9,&vc20_drive9);
...
int vc20_quick_load(char *name)
{
  ...
  fp = osd_fopen(Machine->gamedrv->name, name, 
  ...
  addr+=size;
  vc20_memory[0x31]=vc20_memory[0x2f]=vc20_memory[0x2d]=addr&0xff;
  vc20_memory[0x32]=vc20_memory[0x30]=vc20_memory[0x2e]=addr>>8;
  return 0;
}
...
int vc20_frame_interrupt (void)
{
  ...
  if (!quickload && QUICKLOAD) vc20_quick_load(rom_name[0]);
  quickload=QUICKLOAD;
