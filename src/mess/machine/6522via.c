...
void via_config(int which, const struct via6522_interface *intf)
{
  ...
  via[which].t2lh=0xff;
  via[which].time2=
    via[which].time1=timer_get_time();
}
  ...
  for (i = 0; i < MAX_VIA; i++)
    {
      ...
      UINT8 t2ll=via[i].t2ll, t2lh=via[i].t2lh;
      double time1=via[i].time1, time2=via[i].time2;
      ...
      via[i].intf = intf;
      via[i].t1ll = t1ll; via[i].t1lh = t1lh;
      via[i].t2ll = t2ll; via[i].t2lh = t2lh;
      via[i].time1 = time1;
      via[i].time2 = time2;
    }
}
  ...
  switch (offset)
      ...
      else
	{
	  if ( T1_CONTINUOUS(v->acr) )
	    {
	      val = (TIMER1_VALUE(v)-
		      (TIME_TO_CYCLES(0, timer_get_time()-v->time1)
		       %TIMER1_VALUE(v))-1)&0xff;
	    }
	  else
	    {
	      val = (0x10000-
		     (TIME_TO_CYCLES(0, timer_get_time()-v->time1)&0xffff)
		     -1)&0xff;
	    }
	}
      break;
      ...
      else
	{
	  if (T2_COUNT_PB6(v->acr))
	    ...
	    {
	      val = (0x10000-
		     (TIME_TO_CYCLES(0, timer_get_time()-v->time2)&0xffff)
		     -1)&0xff;
	    }
	}
      break;
	  ...
	  if (v->t2)
	    {
	      if (v->intf->t2_callback)
		v->intf->t2_callback(timer_timeelapsed(v->t2));
	      timer_reset (v->t2, TIME_IN_CYCLES(TIMER2_VALUE(v) + IFR_DELAY,
						 0));
	    }
	  else
	    v->t2 = timer_set (TIME_IN_CYCLES(TIMER2_VALUE(v) + IFR_DELAY, 0),
