...
static int channel,
  tone1pos=0, tone2pos=0, tone3pos=0,
#ifdef NEW_MODELL
  ticks_per_sample,
...
#else
  tonesize,
  tone1samples=1, tone2samples=1, tone3samples=1,
#endif
...
void vic6560_soundport_w(int offset, int data)
{
  ...
  case 0xa:
    ...
    if ((data^old)&0x80) {
      if (TONE1_ON) { tone1count=tone1latch;tone1pos=0;tone1count=0; }
    }
  ...
  case 0xb:
    ...
    if ((data^old)&0x80) {
      if (TONE2_ON) { tone2count=tone2latch;tone2pos=0;tone2count=0; }
    }
  ...
  case 0xc:
    ...
    if ((data^old)&0x80) {
      if (TONE3_ON) { tone3count=tone3latch;tone3pos=0;tone3count=0; }
    }
...
# endif
	...
	noisepos=0;
        ...
        v=(v*VOLUME)<<2;
	    if (v>32767) buffer[i]=32767;
	    else if (v<-32767) buffer[i]=-32767;
	    else buffer[i]=v;
...
int vic6560_custom_start(const struct MachineSound* driver)
{
  ...
  */
  ...
  channel = stream_init("VIC6560", 50, options.samplerate, 8, vic6560_update);
  ...
  if (!noise) return 1;
  {
    ...
    for (i=0; i<noisesize; i++) {
      data=0;
      if (noiseshift&0x400000) data|=0x80;
      if (noiseshift&0x100000) data|=0x40;
      if (noiseshift&0x010000) data|=0x20;
      if (noiseshift&0x002000) data|=0x10;
      if (noiseshift&0x000800) data|=0x08;
      if (noiseshift&0x000080) data|=0x04;
      if (noiseshift&0x000010) data|=0x02;
      if (noiseshift&0x000004) data|=0x01;
      noise[i]=data;
