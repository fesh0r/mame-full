            ...
            for( y = 0; y < 192; y++ )
            {
                ...
                for( x = 0; x < 80; x++, offs++ )
                {
                    if( dirtybuffer[offs] )
                    {
                        ...
                        code = videoram[offs];
                        drawgfx(
							bitmap,Machine->gfx[2],code,color,0,0,sx,sy,
                            &Machine->drv->visible_area,TRANSPARENCY_NONE,0);
                        osd_mark_dirty(sx,sy,sx+7,sy,0);
                        dirtybuffer[offs] = 0;
                    }
                }
            }
            ...
            for( y = 0; y < 192; y++ )
            {
				...
				for( x = 0; x < 40; x++, offs += 2 )
                {
					if( dirtybuffer[offs] || dirtybuffer[offs+1] )
                    {
                        ...
                        drawgfx(
							bitmap,Machine->gfx[3],code,color,0,0,sx,sy,
                            &Machine->drv->visible_area,TRANSPARENCY_NONE,0);
						osd_mark_dirty(sx,sy,sx+15,sy,0);
            ...
            for( y = 0; y < 192; y++ )
            {
				...
				for( x = 0; x < 80; x++, offs++ )
                {
					if( dirtybuffer[offs] )
                    {
                        ...
                        code = videoram[offs];
                        drawgfx(
							bitmap,Machine->gfx[5],code,0,0,0,sx,sy,
                            &Machine->drv->visible_area,TRANSPARENCY_NONE,0);
						osd_mark_dirty(sx,sy,sx+7,sy,0);
						dirtybuffer[offs] = 0;
                    }
			...
			for( y = 0; y < 96; y++ )
            {
				...
				for( x = 0; x < 80; x++, offs++ )
                {
					if( dirtybuffer[offs] )
                    {
                        ...
                        code = videoram[offs];
                        drawgfx(
							bitmap,Machine->gfx[6],code,0,0,0,sx,sy,
                            &Machine->drv->visible_area,TRANSPARENCY_NONE,0);
						osd_mark_dirty(sx,sy,sx+7,sy+1,0);
						dirtybuffer[offs] = 0;
                    }
                }
            }
            ...
            for( y = 0; y < 192; y++ )
            {
				...
				for( x = 0; x < 20; x++, offs += 2 )
                {
					if( dirtybuffer[offs] || dirtybuffer[offs+1] )
                    {
						...
						color = videoram[offs+1];
                        drawgfx(
							bitmap,Machine->gfx[4],code,color,0,0,sx,sy,
                            &Machine->drv->visible_area,TRANSPARENCY_NONE,0);
						osd_mark_dirty(sx,sy,sx+31,sy,0);
						dirtybuffer[offs] = dirtybuffer[offs+1] = 0;
                    }
                }
            }
                ...
                for( x = 0; x < 40; x++, offs += 2 )
                {
                    if( dirtybuffer[0x3800+offs] || dirtybuffer[0x3801+offs] )
                    {
                        ...
                        drawgfx(
                            ...
                            &Machine->drv->visible_area,TRANSPARENCY_NONE,0);
                        osd_mark_dirty(sx,sy,sx+15,sy+7,0);
